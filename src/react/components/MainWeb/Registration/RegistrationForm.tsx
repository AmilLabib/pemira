...
const RegistrationForm: React.FC<Props> = ({ defaultPosisi = "" }) => {
  ...
  const [mbti, setMbti] = useState<File | null>(null);
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (mbti) data.append("mbti", mbti, mbti.name);
    if (formulirDukungan)
      data.append(
    ...
    >
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          ...
          <div>
            <label className="block text-sm font-medium">Prodi / Jurusan</label>
            <select
              value={jurusan}
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div>
            <FileInput
              id="MBTI"
              label="Hasil Tes MBTI — max 10MB"
              accept="image/*,application/pdf"
              file={mbti}
              onFileChange={(e) => handleFileChange(e, setMbti, "mbti")}
              error={errors.mbti}
              showTemplate
              templateHref="https://www.16personalities.com/free-personality-test"
              templateText="Lakukan tes MBTI (16Personalities)"
            />
          </div>
